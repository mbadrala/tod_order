...
import { getBankAccounts, createBankAccount, updateBankAccount, deleteBankAccount, type BankAccount, type BankAccountInput } from '@/lib/api'
...
function BankAccountsPage() {
  ...
  const load = async () => {
    try { setAccounts(await getBankAccounts()) } catch { /* ignore */ }
  }
      ...
      cell: ({ row }: { row: { original: BankAccount } }) => (
        <div className="flex gap-1">
          ...
          <Button variant="outline" size="xs" className="text-destructive"
            onClick={() => handleDelete(row.original.id, row.original.bank_name)}>Устгах</Button>
        </div>
      ),
    }] : []),
  ]
